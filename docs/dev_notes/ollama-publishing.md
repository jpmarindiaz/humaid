# Publishing LFM2.5-VL to Ollama (HuggingFace → Ollama Hub)
...
This is the **base** model (no fine-tune). It's published as a reference for community stations to fall back on. The published Ollama model is **text-only** for now — see [Vision support](#vision-support) below.
...
The flood-detection fine-tune in [`../finetune-flood/`](../finetune-flood/) uses the `llama-server` path because it needs the multimodal capability.
