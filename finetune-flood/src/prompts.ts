// Labeler prompts for change-detection labeling. The labeler (and the
// student model) receives FOUR images: a baseline pair and a current pair.
// Each pair is RGB true color (B4-B3-B2) plus SWIR false color (B12-B8-B4)
// of the same 5km Sentinel-2 tile. The label is for the *current* timestamp's
// flood state, calibrated against the baseline to disambiguate normal water
// bodies (rivers, ciénagas, wetlands) from new flooding.
//
// This pair structure is the central design decision for our flood model.
// Single-tile labeling cannot distinguish "town surrounded by chronic ciénaga
// (La Mojana baseline)" from "town inundated by flood water" — the pixels
// look identical. Pair labeling collapses that ambiguity to "did water
// extent grow vs the baseline?" which is well-defined.
...
export const FLOOD_LABEL_SCHEMA = {
  ...
  properties: {
    ...
    flood_severity: {
      ...
      description:
        "Newly inundated land relative to baseline. 'none'=0%, 'minor'=<5%, 'moderate'=5–20%, 'severe'=>20% of land area newly under water.",
    },
